...
export const loggedInMessage = (user, logOut) => {
  ...
  return (
    <div>
      <p> Logged in as <span className="logged-name">{ firstName }</span> ({ user.email })</p><button onClick={ () => logOut() }>Logout</button>
    </div>
  )
}
...
export const loginMessage = () => {
  return (
    <button className='SignInOut' onClick={ () => signIn() }>Sign In</button>
  )
}
